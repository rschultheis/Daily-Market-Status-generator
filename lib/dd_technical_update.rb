...
module DD_Updater
  ...
  #6 years base data
  ROWS_OF_HISTORY_TO_ANALYZE = 600
  #1 year to prime
  PRIMER_ROWS = 200
  ...
  def update_technical_analysis_csv filename
    ta_filename = File.join(File.dirname(filename), "#{filename.match(/([A-Z^]+)\.csv$/)[1]}_analysis.csv")
    ...
    ((PRIMER_ROWS)...historical_data.length).each do |row|
      ...
      DMA_PERIODS.each do |dma_period|
        ...
        historical_data[row]["#{dma_period}_dma_pct_diff".intern] = (historical_data[row][:adj_close] / dma) - 1.0
      ...
      VMA_PERIODS.each do |vma_period|
        ...
        historical_data[row]["#{vma_period}_vma_pct_diff".intern] = (historical_data[row][:volume] / vma) - 1.0
